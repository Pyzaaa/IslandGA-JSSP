...
int makespan(
    ...
    for (int k = 0; k < m; ++k) machine_ready[k] = 0;
    for (int job : order) {
        int prev = 0;
...
pair<vector<int>, vector<int>> order_crossover(
    ...
    auto ox = [&](const vector<int>& A, const vector<int>& B) {
        ...
        vector<char> used(n, 0);  */ // O(1) membership check
...
vector<int> neh_sequence(const vector<vector<int>>& times) {
    ...
    int m = times[0].size(); //to dodane
    vector<int> machine_ready(m); //to dodane
    int n = times.size();
    vector<int> jobs(n);
...
int main(int argc, char* argv[]) {
    ...
    // ================= prealokowane buffery do crossover =================
    vector<int> child(n);
    vector<char> used(n);
    if (init_mode == "neh") {
          cout << "â†’ Using NEH initialization\n";
    ...
    cout << "â†’ Using RANDOM initialization\n";
    ...
    for (int i = 0; i < pop_size; i++)
        fitness[i] = makespan(population[i], times, machine_ready); //tu machine ready dodane
    ...
    for (int gen = 0; gen < generations; gen++) {
        ...
        while (new_pop.size() < pop_size) {
            ...
            if (rand_double() < cx_prob) {
                auto children = order_crossover(p1, p2, child, used);
        ...
        population = new_pop;
        for (int i = 0; i < pop_size; i++)
            fitness[i] = makespan(population[i], times, machine_ready);//machine ready
