...
__global__ void makespan_kernel(
    ...
    for (int j = 0; j < n; j++) {
        ...
        for (int k = 0; k < m; k++) {
            ...
            int c = start + d_times[job * m + k];  // upewnij się, że flatten_times używa row-major
            mr[k] = c;
            prev = c;
        }
    }
...
__global__ void crossover_mutation(
    const int* parents,
    int* offspring,
    curandState* rng, unsigned char* d_used_all,
    int n, int pop_size,
    double mut_rate
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    // Każdy wątek używa własnej tablicy "used" w global memory
...
int main(int argc, char* argv[]) {
    ...
    for (int gen = 0; gen < generations; gen++) {
        ...
        crossover_mutation<<<blocks, threads>>>(
            d_popA, d_popB, d_rng, d_used_all, n, pop_size, mut_rate
        );
