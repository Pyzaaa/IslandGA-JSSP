...
int tournament_selection(
    ...
    int best = rand_int(0, pop_size - 1);
    for (int i = 1; i < k; i++) {
        int cand = rand_int(0, pop_size - 1);
    ...
    for (int gen = 0; gen < generations; gen++) {
       ...
       /* sort(idx.begin(), idx.end(), [&](int a, int b) {
        ...
        while (new_pop.size() < pop_size) {
            auto i1 = tournament_selection(pop_size, fitness, 3);
            auto i2 = tournament_selection(pop_size, fitness, 3);
            const vector<int>& p1 = population[i1];
            const vector<int>& p2 = population[i2];
...
#pragma omp parallel for
        /*for (int i = 0; i < pop_size; i++) {
            vector<int> tmp(m);
            fitness[i] = makespan(population[i], times, tmp);
        }*/
